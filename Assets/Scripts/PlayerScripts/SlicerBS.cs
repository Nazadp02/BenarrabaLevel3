using EzySlice;
using UnityEngine;
public class SlicerBS : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isTouched == true)
        {
            ...
            foreach (Collider objectToBeSliced in objectsToBeSliced)
            {
                ...
                objectToBeSliced.gameObject.GetComponent<Rigidbody>().velocity);
    ...
    private void MakeItPhysical(GameObject obj, Vector3 _velocity)
    {
        ...
        obj.GetComponent<Rigidbody>().AddForce(3 * new
        Vector3(randomNumberX, randomNumberY, randomNumberZ), ForceMode.Impulse);
